...
import { GradientEntity, GradientLayerEntity, GradientStopEntity } from "../infrastructure/entity/gradient.entity";
...
export const addNewLayer = async (gradientId: number) => {
  let layerId = await invoke("save_layer", { layer: newGradient.layers[0], gradientId: gradientId });
  let layer = await invoke<GradientLayerEntity>("get_layer", {layerId: layerId});
  console.log(layer);
...
export const addNewStop = async (gradientId:number, layerId: number) => {
  let stopId = await invoke("save_stop", { stop: newGradient.layers[0].stops[0], layerId: layerId });
