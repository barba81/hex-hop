...
import { ColorData } from "../Infrastructure/Data/Color.data";
import { ColorLookupName } from "../Colors/colorNameSuggestion";
...
export const AddNewColor = async (color: ColorData) => {
  try {
    const db = getContext();
    const order = GetNextOrderNumber();
    const colorName = await ColorLookupName.getNearestColor(color);
