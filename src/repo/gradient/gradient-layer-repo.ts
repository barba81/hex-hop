...
export const insertGradientLayerAsync = async (gradientLayer: GradientLayerDto) => {
    const db = await getContext();
    ...
    try {
        return await db.execute(`
            ...
            VALUES 
            (?,?,?,?,?,?) `,
            [gradientLayer.gradientId, gradientLayer.gradientType, gradientLayer.rotationDegree, gradientLayer.patternRepeatNumber, gradientLayer.colorSpace, gradientLayer.easingFunction]);
    } catch (error) {
